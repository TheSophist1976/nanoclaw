...
1. Query Athenaeum for the recall queue and current reading state: `mcp__athenaeum__get_context("Scholar recall queue and current reading", verbosity: "standard")`. Identify overdue items (past their next review date or confidence 1–2 reviewed 2+ days ago).
...
7. After the session, save the updated recall queue state to Athenaeum: `mcp__athenaeum__add_memory(content, tags: ["domain:learning", "agent:scholar", "type:recall-queue"], content_type: "durable")`. Include all item names, updated confidence scores, last reviewed date, and next review date. If an item hit confidence 3 for the second consecutive time, mark it Retired in the saved state.
